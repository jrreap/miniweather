...
int main(int argc, char **argv) {
  ...
  sim_time = 700;     //How many seconds to run the simulation
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  #pragma acc parallel loop collapse(2) private(ll, s, inds, vals, stencil, d3_vals, r,u,w,t,p)
  for (k=0; k<nz; k++) {
    for (i=0; i<nx+1; i++) {
  ...
  #pragma acc parallel loop collapse(3) private(indt, indf1, indf2, ll, k, i)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
        indt  = ll* nz   * nx    + k* nx    + i  ;
...
void compute_tendencies_z( double *state , double *flux , double *tend ) {
  ...
  /////////////////////////////////////////////////
  //Compute fluxes in the x-direction for each cell
  #pragma acc parallel loop collapse(2) private(ll,s,k,i, inds, vals, stencil, d3_vals, r,u,w,t,p)
  for (k=0; k<nz+1; k++) {
    for (i=0; i<nx; i++) {
      //Use fourth-order interpolation from four cell averages to compute the value at the interface in question
      for (ll=0; ll<NUM_VARS; ll++) {
  ...
  /////////////////////////////////////////////////
  //Use the fluxes to compute tendencies for each cell
  #pragma acc parallel loop collapse(3) private(ll,k,i, indt, indf1, indf2)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
        indt  = ll* nz   * nx    + k* nx    + i  ;
...
void set_halo_values_x( double *state ) {
  ...
  //////////////////////////////////////////////////////
  #pragma acc parallel loop collapse(2) private(ll, k)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
...
void set_halo_values_z( double *state ) {
  ...
  // TODO: THREAD ME
  /////////////////////////////////////////////////
  #pragma acc parallel collapse(2) loop private(ll,i)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
